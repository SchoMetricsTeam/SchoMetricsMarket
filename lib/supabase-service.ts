...
export const uploadAvatarToSupabase = async (
  ...
  const uniqueFileName = generateUniqueFileName(originalFileName);
  const fileKey = `${userTypePrefix}/${identifierPrefix}/${folderPrefix}${uniqueFileName}`;
  const format = originalFileName.split(".").pop()?.toLowerCase() || "";
...
export const uploadTransferBaucherImage = async (
  buffer: Buffer,
  originalFileName: string,
  mimeType: string,
  userTypePrefix: string,
  identifierPrefix: string,
  folderPrefix = "trasfer-baucher-image/"
): Promise<{
  fileKey: string;
  publicUrl: string;
  fileName: string;
  ...
  try {
    ...
    if (error) {
      console.error("Error uploading baucher to Supabase:", error);
      throw new Error("Error al subir el baucher de transferencia");
    }

    const publicUrl = `${supabaseStorageUrl}/${fileKey}`;
    console.log("Baucher subido a Supabase Storage. Key:", fileKey);
    ...
    throw new Error("Error al subir el archivo de baucher de transferencia");
  }
};
...
export const deleteUserFolderFromSupabase = async (
  ...
  const folderPrefix = `${userTypePrefix}/${identifierPrefix}/`;
